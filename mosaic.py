...
# Parameters that determine how the mosaic is created
...
TILE_BLOCK_SIZE = TILE_SIZE / max(min(TILE_MATCH_RES, TILE_SIZE), 1) # size of the blocks that make up each tile
WORKER_COUNT = max(cpu_count() - 1, 1) 	# number of worker processes to run in parallel
OUT_FILE = 'mosaic.jpeg'
EOQ_VALUE = None

class TileProcessor: # Worker process
	...
	def __process_tile(self, tile_path):
		try:
			...
			# get the largest square that fits inside the image and crop the image to that
			w = img.size[0]
			h = img.size[1]
			...
			return (large_tile_img.convert('RGB'), small_tile_img.convert('RGB')) # convert to RGB for faster processing
		except:
			return (None, None)
	...
	def get_tiles(self):
		...
		print('Reading tiles from {}...'.format(self.tiles_directory))
 		...
 		# for each file found, process it to create a large and small version of the tile
...
class TargetImage: # Main process
	...
	def get_data(self):
		...
		# crop the image to make it an exact multiple of the tile size
		if w_diff or h_diff:
...
class TileFitter:	# Main process
	...
	def get_best_fit_tile(self, img_data):
		...
		# find the tile that best matches the image data
...
class MosaicImage: 
	...
	# add a tile to the mosaic image
...
def compose(original_img, tiles):
	...
	try:
		...
		# put the work into the work queue
		...
		# put the end-of-queue markers into the work queue
		for n in range(WORKER_COUNT):
